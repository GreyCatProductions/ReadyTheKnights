...
import { CELL_SIZE, BUILDING_OBSTACLE_RADIUS, TREE_OBSTACLE_RADIUS } from "../../shared/Constants";
    ...
    app.ticker.add(() => {
        ...
        state.nodes.forEach((node) => {
            ...
            node.buildings.forEach((b) => {
                ...
                g.circle(cx, cy, BUILDING_OBSTACLE_RADIUS).stroke({ color: 0xff4400, alpha: 0.3, width: 1 });
            ...
            node.worldObjects.forEach((obj) => {
                ...
                g.circle(cx, cy, TREE_OBSTACLE_RADIUS).stroke({ color: 0x00cc44, alpha: 0.3, width: 1 });
                debugLayer!.addChild(g);
            });
        });
    });
